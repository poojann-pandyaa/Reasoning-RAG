...
NOTE: bitsandbytes 4-bit quantization only works on CUDA (Linux/Windows GPU).
      On macOS (MPS/CPU), training runs in fp32 -- very slow.
...
def load_base_model(device: str):
    ...
    if device == "cuda":
        ...
        print("          4-bit quantization (bitsandbytes) requires CUDA.")
        print("          Training on MPS/CPU in fp32 -- VERY slow.")
        print("          Strongly recommended: use Google Colab (free T4 GPU).")
        model = AutoModelForCausalLM.from_pretrained(
            MODEL_NAME,
...
def main():
    ...
    # 6. Save adapter only (~50-100 MB)
    trainer.model.save_pretrained(args.output)
    tokenizer.save_pretrained(args.output)
    print(f"\nLoRA adapter saved to: {args.output}")
    print("\nTo use in inference, pass lora_adapter_path to FinalGenerator:")
