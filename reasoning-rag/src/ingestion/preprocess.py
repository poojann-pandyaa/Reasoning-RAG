...
def run_preprocessing(max_samples=None, output_path="data/processed_dataset.jsonl"):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for record in dataset:
            ...
            # Filter: at least one answer with score >= 5 OR an accepted answer
            valid = any(
                ans.get("pm_score", 0) >= 5 or ans.get("selected", False)
                for ans in answers
            )
            if not valid:
                continue
            ...
            # Build a title: first 120 chars of the cleaned question (no HTML)
            # This was previously hardcoded to '' which left Q: fields blank in the index.
            title = clean_question[:120].strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-samples",
        type=int,
        default=os.environ.get("MAX_SAMPLES"),
